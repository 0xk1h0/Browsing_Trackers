...
proto/spawn_session.py      (per-PID session orchestrator with file lock)
   |
   |-- mitmdump              (per-session; runs agentcloak addon + stealth addon)
...
The mitmproxy capture addon is shared with the agent measurement pipeline so
schema parity is enforced by reuse:

- `agentcloak/measurement/mitm_capture_addon.py` — relative to the repo root.

`proto/spawn_session.py` resolves this path in the following order:

1. `USER_STUDY_ADDON_PATH` environment variable (explicit override).
2. Repo-default at `../../../agentcloak/measurement/mitm_capture_addon.py`
   relative to the user-study folder (the standard checkout layout).
3. `$AGENTCLOAK_ROOT/measurement/mitm_capture_addon.py` if the env var is set.
