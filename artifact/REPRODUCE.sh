...
#
# Runs every per-RQ `scripts/run_repro.sh` in sequence. Shipped data only — no GPU,
# no mitmproxy, no API calls. Expected wall-clock: well under a minute.
#
# Usage:
...
for module in "${selected[@]}"; do
  ...
  # Checked for existence, not the executable bit: a zip download or a
  # checkout on a filesystem without mode bits must still reproduce.
  if [[ ! -f "$script" ]]; then
    echo "[REPRODUCE] missing: $script (skip)" >&2
    fail=1
    continue
  fi
  echo
  ...
  echo "  $module"
  echo "================================================================"
  # Stamp the module before it runs. verify.py treats any checked output
  # older than this stamp as not regenerated, so a module that fails part
  # way cannot pass verification on the copies shipped in the checkout.
  touch "$module/.reproduced"
