...
| 3 | `pipeline/aggregation/six_metrics.py` | Reduces a session to six counts (M_P, M_T, M_X, M_F, behavior signals, direct-identifier signals) using the keyword sets in `family_classifier.py`. Outputs one row per session — the Tier-1 CSV every RQ analyzer consumes. |
...
643-task WebVoyager benchmark (paired against scripted same-site baselines).
Per-session aggregates are binned by cross-site-transition count for the
dose-response plot; per-agent paired analyses (vs. scripted baseline) run
the Wilcoxon-equivalent rank tests with Holm correction.
...
bash scripts/run_repro.sh         # runs all 6 analyzers + the dose-response plot
# or one at a time:
...
| §7         | `analyze_paired.py`                            | Per-agent ITT paired vs. scripted same-site                  |
...
19 checks run: 15 numeric diffs over the regenerated TSV, CSV, and JSON
