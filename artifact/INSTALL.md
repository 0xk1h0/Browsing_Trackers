...
Two virtual environments, because the Browser-Use harness and the vLLM stack
pin incompatible versions of shared libraries. Python 3.12 or 3.13 for both.

```bash
# 1. Capture proxy, agent runners, Browser-Use harness, OpenAI client
bash ../install.sh --full       # requirements-full.txt + playwright chromium
source .venv/bin/activate

# 2. Model serving, used only by agents/*/serve.sh
uv venv ../.venv-serve && uv pip install --python ../.venv-serve/bin/python -r requirements-serve.txt
# or: python3 -m venv ../.venv-serve && ../.venv-serve/bin/pip install -r requirements-serve.txt
```

Run `serve.sh` from the serving environment (`source ../.venv-serve/bin/activate`
in that shell) and everything else from `.venv`.
