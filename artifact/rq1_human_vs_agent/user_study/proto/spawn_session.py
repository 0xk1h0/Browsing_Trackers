...
def _resolve_addon_path() -> Path:
    """Locate the shared mitmproxy capture addon used by both the user study
    and the agent measurement pipeline. Search order:
      1. USER_STUDY_ADDON_PATH env var (explicit override for reviewers).
      2. ../../../agentcloak/measurement/mitm_capture_addon.py relative to
         this file (the default repo layout).
      3. AGENTCLOAK_ROOT/measurement/mitm_capture_addon.py if AGENTCLOAK_ROOT
         is set.
    """
    ...
    repo_default = (
        ...
        / "agentcloak" / "measurement" / "mitm_capture_addon.py"
    )
    ...
    return repo_default  # may not exist; surfaced as a clear error at start
