...
def stale(module: str, path: Path) -> bool:
    """True when REPRODUCE.sh stamped this module after `path` was last written:
    the last run did not regenerate it, so what is on disk is the shipped copy."""
    stamp = HERE / module / ".reproduced"
    return stamp.exists() and path.stat().st_mtime < stamp.stat().st_mtime


STALE_MSG = "not regenerated by the last REPRODUCE.sh run (it reported an error for this module)"
...
def check_table(module: str, rel: str) -> tuple[bool, str, list[str]]:
    ...
    if stale(module, observed):
        return False, label, [STALE_MSG]
